...
bool TextureManager::Load(std::string id, std::string filename) {
    ...
    SDL_Texture* texture = SDL_CreateTextureFromSurface(Engine::getInstance()->getMainWindow()->getRenderer(), surface);
    if(texture == nullptr){
...
void TextureManager::Draw(std::string id, int x, int y, int width, int heigt, SDL_RendererFlip flip) {
    ...
    SDL_RenderCopyEx(Engine::getInstance()->getMainWindow()->getRenderer(),m_TextureMap[id], &srcRect, &dstRect,0, nullptr, flip);
}
